...
public:
    ...
    SharedPointer<Array<Edge<E>>> Prim(const E& LIMIT,const bool MINMAX = true)
    {
        ...
        if(asUndirected())
        {
            ...
            for (int i = 0;(i < vCount()) && !end;i++)
            {
                ...
                if(!end)
                {
                    ...
                    for (int j = 0;j < aj->length();j++)
                    {
                        if(!mark[(*aj)[j]] && (MINMAX ? (getEdge(k,(*aj)[j]) < cost[(*aj)[j]]) :
    ...
    SharedPointer<Array<Edge<E>>> Kruskal(const bool MINMAX = true)
    {
        LinkQueue<Edge<E>> ret;
